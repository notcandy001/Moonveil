...
# lib/common.sh  —  Moonveil Shared Library
...
# · Logging helpers (info, success, warn, error, step, pkg_*)

# · Package install engine with live per-package display

# · Backup / dotfile apply helpers
...
# ── Step counter (set by installers) ──────────────────────────────────────────
...
# Numbered section header with a box

mv_step() {
MV_STEP_CURRENT=$(( MV_STEP_CURRENT + 1 ))
local num; printf -v num “%02d” “$MV_STEP_CURRENT”
local total; printf -v total “%02d” “$MV_STEP_TOTAL”
echo “”
echo -e “  ${PURPLE}${B}┌─────────────────────────────────────────────────────┐${R}”
echo -e “  ${PURPLE}${B}│  ${LPURPLE}Step ${num}/${total}  ${WHITE}${B}$*${PURPLE}${B}                                    │${R}” 2>/dev/null ||   
echo -e “  ${PURPLE}${B}│  ${LPURPLE}Step ${num}/${total}  ${WHITE}${B}$*  ${PURPLE}${B}│${R}”
echo -e “  ${PURPLE}${B}└─────────────────────────────────────────────────────┘${R}”
echo “”
}
...
# Call before a batch: mv_pkg_start <total> <label>
...
# Show a package as “installing”

mv_pkg_show() {
local name=”$1”
local source=”$2”   # pacman | aur | apt | dnf | pip | flatpak
...
# Overwrite the last line with a done/skip/fail result
...
# GENERIC INSTALL WRAPPERS

# (Each distro installer passes its own install_fn)

# ══════════════════════════════════════════════════════════════════════════════

# mv_install_pkg <name> <source> <already_installed_fn> <install_fn> [args…]

# already_installed_fn: bash function that returns 0 if pkg installed, 1 if not

# install_fn: bash function that installs the package
...
mv_info “Applying ~/.config…”
cp -r “${install_dir}/dots/.config/”* “$HOME/.config/” 2>/dev/null || true
mv_success “~/.config applied”
else
mv_warn “dots/.config not found in repo”
fi

if [[ -d “${install_dir}/dots/.local” ]]; then
mv_info “Applying ~/.local…”
cp -r “${install_dir}/dots/.local/”* “$HOME/.local/” 2>/dev/null || true
mv_success “~/.local applied”
else
mv_warn “dots/.local not found in repo”
fi
}
...
fc-cache -fv >> “${MV_LOG_FILE:-/tmp/moonveil.log}” 2>&1
